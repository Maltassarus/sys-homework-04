...
Таблицы на разных серверах. Маршрутизируются по имени таблицы. Если мастер падает, то слейв повышается до мастера.

Сервер 1 (users) режим - active master:	принимает запросы на запись и чтение по пользователям;

Сервер 1 (users_slave) - slave:	копия для резервирования и отказоустойчивости;

Сервер 2 (books) - active master: принимает запросы на запись и чтение по книгам;

Сервер 2 (books_slave) - slave: копия для резервирования;

Сервер 3 (shops) - active master: принимает запросы на запись и чтение по магазинам;

Сервер 3 (shops_slave) - slave: копия для резервирования.


Маршрутизатор определяет, на какой сервер отправить запрос.